...
def compute_drawdown(cum_returns: pd.Series) -> pd.Series:
    ...
    return 1 - (cum_returns / running_max)
...
def update_heatmaps(selected_heatmap_strategies):
    ...
    for the chosen strategies. (Uses entire returns_df, ignoring the date range.)
    """
    ...
    weekly_returns = (1 + returns_df).resample('W-MON').prod() - 1
